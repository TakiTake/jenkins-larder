...
- [x] T013 [P] Write unit tests for CachedPlugin serialization in tests/unit/storage_test.go
...
- [x] T016 [P] Write unit tests for checksum validation in tests/unit/checksum_test.go
...
- [x] T019 [P] Write unit tests for URL construction in tests/unit/upstream_test.go
...
- [x] T023 [P] [US1] Contract test for plugin download URL pattern in tests/contract/plugin_api_test.go
- [x] T024 [P] [US1] Contract test for checksum file download in tests/contract/plugin_api_test.go
- [x] T025 [P] [US1] Integration test for cache miss flow (download, store, serve) in tests/integration/download_test.go
- [x] T026 [P] [US1] Integration test for cache hit flow (serve from disk) in tests/integration/download_test.go
- [x] T027 [P] [US1] Integration test for LRU eviction when storage full in tests/integration/download_test.go
- [x] T028 [P] [US1] Integration test for concurrent requests to same uncached plugin in tests/integration/download_test.go
...
- [x] T040 [US1] Implement graceful upstream failure handling (serve stale with warning log) in src/server/cache.go
...
- [x] T042 [P] [US2] Contract test for admin invalidation API endpoint in tests/contract/admin_api_test.go
- [x] T043 [P] [US2] Integration test for manual cache invalidation in tests/integration/invalidate_test.go
- [x] T044 [P] [US2] Integration test for optional TTL policy (if TTL enabled, refresh after expiry) in tests/integration/invalidate_test.go

### Implementation for User Story 2

- [x] T045 [P] [US2] Implement invalidation logic (delete plugin files and metadata) in src/server/cache.go (InvalidatePlugin)
- [x] T046 [P] [US2] Implement TTL checking (if enabled, check age vs configured TTL) in src/storage/plugin.go (IsStale)
- [x] T047 [US2] Implement HTTP handler for POST /admin/cache/invalidate in src/admin/handlers.go
- [x] T048 [US2] Implement request validation for invalidation API in src/admin/handlers.go
- [x] T049 [US2] Implement HTTP handler for GET /admin/cache/stats in src/admin/handlers.go
- [x] T050 [US2] Implement HTTP handler for GET /admin/health in src/admin/handlers.go
- [x] T051 [US2] Wire admin API routes to HTTP server on port 8081 in src/server/server.go (AdminHandler)
...
- [x] T052 [P] [US3] Contract test for Prometheus metrics endpoint format in tests/contract/metrics_test.go
- [x] T053 [P] [US3] Integration test for metrics collection (verify hit rate calculation) in tests/contract/metrics_test.go (TestMetricsCacheHitRatioContract)

### Implementation for User Story 3

- [x] T054 [P] [US3] Initialize Prometheus registry and collectors in src/metrics/metrics.go (promauto)
- [x] T055 [P] [US3] Create mirror_requests_total counter with status label in src/metrics/metrics.go (PluginDownloads)
- [x] T056 [P] [US3] Create mirror_request_duration_seconds histogram with status label in src/metrics/metrics.go (DownloadDuration)
- [x] T057 [P] [US3] Create mirror_storage_bytes_used and mirror_storage_bytes_limit gauges in src/metrics/metrics.go
- [x] T058 [P] [US3] Create mirror_cached_plugins_total gauge in src/metrics/metrics.go (CachedPluginsTotal)
- [x] T059 [P] [US3] Create mirror_bandwidth_saved_bytes_total counter in src/metrics/metrics.go (BandwidthSavedBytes)
- [x] T060 [P] [US3] Create mirror_evictions_total counter with reason label in src/metrics/metrics.go (EvictionsTotal)
- [x] T061 [P] [US3] Create mirror_upstream_failures_total and mirror_checksum_failures_total counters in src/metrics/metrics.go
- [x] T062 [US3] Integrate metrics updates into cache hit/miss logic in src/server/cache.go
- [x] T063 [US3] Integrate metrics updates into LRU eviction logic in src/server/cache.go (ensureSpace)
- [x] T064 [US3] Implement HTTP handler for GET /metrics (Prometheus exposition) in src/server/server.go (promhttp.Handler)
- [x] T065 [US3] Wire metrics endpoint to HTTP server on port 9090 in src/server/server.go
...
- [x] T066 [P] Create Kubernetes PersistentVolumeClaim manifest in deployments/kubernetes/pvc.yaml
- [x] T067 [P] Create Kubernetes ConfigMap manifest in deployments/kubernetes/configmap.yaml
- [x] T068 [P] Create Kubernetes Deployment manifest with health/readiness probes in deployments/kubernetes/deployment.yaml
- [x] T069 [P] Create Kubernetes Service manifest (ports 8080, 8081, 9090) in deployments/kubernetes/service.yaml
- [x] T070 [P] Create Kubernetes ServiceMonitor for Prometheus Operator in deployments/kubernetes/servicemonitor.yaml
- [x] T071 [P] Create Dockerfile for Go binary (multi-stage build, Alpine base) in Dockerfile
- [x] T072 [P] Create main.go entry point (load config, start server, handle signals) in cmd/larder/main.go
- [x] T073 [P] Add end-to-end test in tests/integration/e2e_test.go
- [x] T074 [P] Create GitHub Actions CI workflow (build, test, lint) in .github/workflows/ci.yaml
- [x] T075 [P] Create README.md with quickstart deployment instructions
...
### ✅ Completed
- **Phase 1: Setup** - All 7 tasks complete (T001-T007)
- **Phase 2: Foundational** - All 15 tasks complete (T008-T022)
- **Phase 3: User Story 1** - All 19 tasks complete (T023-T041)
- **Phase 4: User Story 2** - All 10 tasks complete (T042-T051)
- **Phase 5: User Story 3** - All 14 tasks complete (T052-T065)
- **Phase 6: Polish** - 10 of 11 tasks complete (T066-T075, T076 pending K8s validation)

### 🧪 Tested & Verified (all passing)
- Config loading & validation: ✅ 12 unit tests
- Storage operations: ✅ 10 unit tests
- Checksum validation: ✅ 5 unit tests
- URL construction & parsing: ✅ 11 unit tests
- LRU tracking & eviction: ✅ 8 unit tests
- Plugin API contract: ✅ 6 contract tests
- Admin API contract: ✅ 3 contract tests (invalidation, stats, health)
- Metrics contract: ✅ 7 contract tests (endpoint format, counters, gauges, histograms)
- Integration flows: ✅ 6 integration tests (cache miss/hit, eviction, upstream failure, concurrent dedup)
- Admin integration: ✅ 2 integration tests (manual invalidation, TTL policy)
- End-to-end: ✅ 1 E2E test (7 sub-tests covering full workflow)

### 🎯 Status: FEATURE COMPLETE

The server provides:
- Plugin caching with LRU eviction (US1)
- Admin API: invalidation, stats, health check (US2)
- Prometheus metrics: hits, misses, duration, storage, bandwidth saved, evictions (US3)
- Docker image, K8s manifests, CI pipeline, README (Phase 6)

### 📋 Remaining
- T076: K8s deployment validation (requires cluster access)
